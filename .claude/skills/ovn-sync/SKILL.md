...
```bash
# Fetch branches
git fetch origin release-4.23 release-4.22 release-4.21 release-4.20 release-4.19 release-4.18 master

# Define sync pairs to check
BUNDLE_1_PAIRS=(
  "release-4.23:release-4.22"
...
### 2. Download CI Job Analysis Scripts

Download the latest versions of the job status scripts:
...
# Download scripts from pr-ci-dashboard repo
curl -fsSL https://raw.githubusercontent.com/jluhrsen/pr-ci-dashboard/main/scripts/e2e-retest.sh -o "$SCRIPT_DIR/e2e-retest.sh"
curl -fsSL https://raw.githubusercontent.com/jluhrsen/pr-ci-dashboard/main/scripts/payload-retest.sh -o "$SCRIPT_DIR/payload-retest.sh"
curl -fsSL https://raw.githubusercontent.com/jluhrsen/pr-ci-dashboard/main/scripts/common.sh -o "$SCRIPT_DIR/common.sh"

# Make executable
chmod +x "$SCRIPT_DIR"/*.sh
```

**CRITICAL:** These scripts distinguish between:
- **E2E jobs**: GitHub status checks matching `ci/prow/(.*e2e|qe-)`
- **Payload jobs**: Jobs triggered via `/payload-job` command, tracked at `pr-payload-tests.ci.openshift.org`

### 3. For Each Sync Pair
...
#### E. Check CI Job Status for Sync PR

For each open sync PR, analyze both e2e and payload job status:

```bash
# Run e2e job analysis (returns JSON)
E2E_STATUS=$("$SCRIPT_DIR/e2e-retest.sh" --json openshift/ovn-kubernetes $PR_NUMBER)

# Run payload job analysis (returns JSON)
PAYLOAD_STATUS=$("$SCRIPT_DIR/payload-retest.sh" --json openshift/ovn-kubernetes $PR_NUMBER)

# Parse results
E2E_FAILED=$(echo "$E2E_STATUS" | jq -r '.failed[] | .name')
E2E_RUNNING=$(echo "$E2E_STATUS" | jq -r '.running[] | .name')
PAYLOAD_FAILED=$(echo "$PAYLOAD_STATUS" | jq -r '.failed[] | .name')
PAYLOAD_RUNNING=$(echo "$PAYLOAD_STATUS" | jq -r '.running[] | .name')
```
...
{
  "failed": [
    {"name": "e2e-azure-ovn-upgrade", "consecutive": 2, "urls": ["url1", "url2"]},
    {"name": "e2e-aws-ovn-fdp-qe", "consecutive": 1, "urls": ["url1"]}
  ],
  "running": [
    {"name": "e2e-gcp-ovn-upgrade"}
  ]
}
```
...
{
  "failed": [
    {"name": "periodic-ci-openshift-release-main-ci-5.0-e2e-azure-ovn-upgrade", "consecutive": 3, "urls": ["url1", "url2", "url3"]},
    {"name": "periodic-ci-openshift-release-main-nightly-5.0-e2e-aws-ovn-upgrade-fips", "consecutive": 1, "urls": ["url1"]}
  ],
  "running": [
    {"name": "periodic-ci-openshift-release-main-ci-5.0-e2e-gcp-ovn-upgrade"}
  ]
}
```
...
### CI Job Status:
**E2E Jobs:**
- ❌ 2 failed: e2e-azure-ovn-upgrade (2 consecutive), e2e-aws-ovn-fdp-qe
- ⏳ 1 running: e2e-gcp-ovn-upgrade

**Payload Jobs:**
- ❌ 2 failed: periodic-ci-openshift-release-main-ci-5.0-e2e-azure-ovn-upgrade (3 consecutive), periodic-ci-openshift-release-main-nightly-5.0-e2e-aws-ovn-upgrade-fips
- ⏳ 1 running: periodic-ci-openshift-release-main-ci-5.0-e2e-gcp-ovn-upgrade
```

**IMPORTANT:**
- E2E job names are short (e.g., `e2e-azure-ovn-upgrade`)
- Payload job names are long (e.g., `periodic-ci-openshift-release-main-ci-5.0-e2e-azure-ovn-upgrade`)
- Show consecutive failure counts when > 1
- If no failures and no running jobs, show: "✅ All CI jobs passing"
- If no payload runs found, show: "⚠️ No payload jobs triggered yet"

#### F. Get Merged PRs to Source Branch Since Last Sync
...
### CI Job Status:
**E2E Jobs:**
- ❌ 2 failed: e2e-azure-ovn-upgrade (2 consecutive), e2e-metal-ipi-ovn-dualstack-bgp-local-gw
- ⏳ 1 running: e2e-gcp-ovn-upgrade

**Payload Jobs:**
- ❌ 2 failed: periodic-ci-openshift-release-main-ci-5.0-upgrade-from-stable-4.22-e2e-gcp-ovn-rt-upgrade (3 consecutive)
- ⏳ 0 running
...
### 5. Special Case: Downstream Sync (Upstream Master → Downstream Master)
...
**CRITICAL:** Downstream sync PRs often have BOTH e2e and payload jobs. Always check both using the scripts from step 2.
